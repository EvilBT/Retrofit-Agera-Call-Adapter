...
/**
 * 文 件 名: SimpleResultRepository
 * 创 建 人: 陈志鹏
 * 创建日期: 2016/11/12 00:43
 * 邮   箱: ch_zh_p@qq.com
 * 修改时间:
 * 修改备注:
 */

class SimpleResultRepository<T> implements ResultRepository<T>{
    ...
    SimpleResultRepository(@NonNull Call<T> call) {
        ...
        mExecCall.enqueue(new Callback<T>() {
            ...
            public void onResponse(Call<T> call, Response<T> response) {
                if (response.isSuccessful()){
                    mResult = Result.success(response.body());
                }else{
                    mResult = Result.failure(new HttpException(response));
                }
